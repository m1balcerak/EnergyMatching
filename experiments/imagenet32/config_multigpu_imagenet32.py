...
def define_flags():
    ...
    flags.DEFINE_float("energy_clamp", 10000,
    ...
    flags.DEFINE_integer("total_steps", 2500001, "Total training steps")
    ...
    # Evaluation / Saving
    flags.DEFINE_integer("save_step", 250, "Checkpoint save frequency (0=disable)")
    flags.DEFINE_string("resume_ckpt", "", "Path to checkpoint for resuming training")

    # EBM + CD
    flags.DEFINE_float("epsilon_max", 0.00, "Max step size in Gibbs sampling")
    flags.DEFINE_float("dt_gibbs", 0.005, "Step size for Gibbs sampling")
    flags.DEFINE_integer("n_gibbs", 0, "Number of Gibbs steps")
    flags.DEFINE_float("lambda_cd", 0., "Coefficient for contrastive divergence loss") 
